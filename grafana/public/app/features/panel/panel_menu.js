...
function (angular, $, _, Tether) {
  ...
  angular
    ...
    .directive('panelMenu', function($compile, linkSrv) {
      ...
      function createMenuTemplate(ctrl, $scope) {
        var template = '<div class="panel-menu small">';
        var roleIsViewer = $scope.$root.contextSrv.user.orgRole === 'Viewer';
        ...
        _.each(ctrl.getMenu(), function(item) {
          ...
          //修改start：添加if判断不显示‘Share’
          var panelMenuText = item.text;
          if(item.text === 'Share') {
            panelMenuText = '分享';
          } else if(item.text === 'View') {
            panelMenuText = '查看';
          } else if(panelMenuText === 'Edit') {
            panelMenuText = '编辑';
          } else if(panelMenuText === 'Duplicate') {
            panelMenuText = '复制';
          }
          if(!(roleIsViewer && item.text === 'Share')) {
            template += '<a class="panel-menu-link" ';
            if (item.click) { template += ' ng-click="' + item.click + '"'; }
            if (item.href) { template += ' href="' + item.href + '"'; }
            template += '>';
            template += panelMenuText + '</a>';
          }
          //修改end：添加if判断不显示‘Share’
        });
        ...
        link: function($scope, elem) {
          ...
          var showMenu = function(e) {
            ...
            if ($(e.target).hasClass('fa-external-link')) {
              ...
              menuTemplate = createMenuTemplate(ctrl, $scope);
            }
